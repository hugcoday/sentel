...
func (t *TCP) SerializeTo(b gopacket.SerializeBuffer, opts gopacket.SerializeOptions) error {
	...
	//log.Println("optionLength", optionLength)
	if opts.FixLengths {
		if rem := optionLength % 4; rem != 0 {
			//log.Println("if  optionLength % 4 ")
			t.Padding = lotsOfZeros[:4-rem]
		}
		//t.DataOffset = uint8((len(t.Padding) + optionLength + 20) / 4)
		t.DataOffset = uint8((optionLength + 20) / 4)
	}
	//log.Println("t.DataOffset", t.DataOffset)
	...
	start := 20
	for _, o := range t.Options {
		//log.Println("before o.OptionLength", o.OptionLength)
		...
		default:
			...
			//log.Println("latar o.OptionLength", o.OptionLength)
		}
	}
